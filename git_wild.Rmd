---
title: "Week 3 Discussion File"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(tidyverse)
```

## Part A: Make edits (no conflict)

```{r partner_1a}
# Partner 1: 
# Run the code below to see the original graph
# Change the code below to make the point color change based on 'cut', and update the theme to theme_bw(). Update the title to include your first name. 

ggplot(data = diamonds, aes(x = carat, y = price)) +
  geom_point(aes(color = cut)) +
  theme_bw() +
  labs(title = "Dylan was here") 

# Changed a few thangs. dddd
```

```{r partner_2a}
# Partner 2: 
# Run the code below to see the original graph
# Change the code below to make the geom a geom_jitter, and update the theme to theme_light. Update the title to include your first name. 

ggplot(data = diamonds, aes(x = color, y = carat)) +
  geom_jitter() +
  theme_light() +
  labs(title = "Shane was here")
```

## PART B: Make problems

```{r break-it-1}
ggplot(data = diamonds, aes(x = depth, y = carat, color = carat)) +
  geom_jitter() +
  theme_classic()

## Break it all down 

ggplot(data = diamonds, aes(x = depth, y = carat)) +
  geom_point() +
  theme_void()
### yabada dabbaggg 
```

```{r break-it-2}

view(iris)

## Lets do it again 

ggplot(data = diamonds, aes(x = carat, y = price, color = color)) +
  geom_point()+
  theme_classic()

## oh yeah 

# I decided your graph was great, so we keep it all!

```

## END Discussion 3
